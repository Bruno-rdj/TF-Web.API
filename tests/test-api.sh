...
RESPONSE=$(curl -s -X POST $API_URL/clientes -H "Content-Type: application/json" -d '{
  "nome": "Cliente Teste",
  "cpf": "529.982.247-25",
...
# Extrair o código do cliente da resposta
...
curl -s -X PUT $API_URL/clientes/$CODIGO -H "Content-Type: application/json" -d '{
...
echo -e "\n7. Excluindo cliente..."
curl -s -X DELETE $API_URL/clientes/$CODIGO
echo "Cliente excluído"

echo -e "\n8. Verificando se cliente foi excluído..."
curl -s -X GET $API_URL/clientes/$CODIGO
